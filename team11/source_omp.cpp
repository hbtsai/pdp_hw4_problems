...
float p0_global; 
float p1_global; 
  double rc2_global;           //Rcut*Rcut
  int npart_global;
  double cell_global[3]; //unit cell length (A)
  bool period_global;
  */
...
//	rc2_global=rc2;
//	npart_global=npart;
//	cell_global[0]=cell[0];
//	cell_global[1]=cell[1];
//	cell_global[2]=cell[2];
//	period_global=period;
    Ep=0;
#pragma omp parallel default(none) private(i, j) //firstprivate(p0_global, p1_global, rc2_global, npart_global, cell_global, period_global)
{
#pragma omp for 
    for(i=0;i<npart;i++) {
        for(j=i+1;j<npart;j++) {
    ...
    double nbox;
            /*for(k=0;k<3;k++)*/ xr[0]= x[i][0] - x[j][0];  //distance vector
		xr[1]= x[i][1] - x[j][1];
		xr[2]= x[i][2] - x[j][2];
...
//              for(k=0;k<3;k++) {
                    f[i][0]+= ff*xr[0];
                    f[j][0]-= ff*xr[0];  //Newton's 3rd law
                    f[i][1]+= ff*xr[1];
                    f[j][1]-= ff*xr[1];  //Newton's 3rd law
                    f[i][2]+= ff*xr[2];
                    f[j][2]-= ff*xr[2];  //Newton's 3rd law
 //             }
              //the stress tensor
...
//        for(k=0;k<3;k++) {
           ...
           x[i][0] +=  v[i][0]*dt + f[i][0]*dt*dt*418.4/(2*m[i]);
           fm[i][0]= f[i][0];
           x[i][1] +=  v[i][1]*dt + f[i][1]*dt*dt*418.4/(2*m[i]);
           fm[i][1]= f[i][1];
           x[i][2] +=  v[i][2]*dt + f[i][2]*dt*dt*418.4/(2*m[i]);
           fm[i][2]= f[i][2];
//        }
    ...
    for(i=0;i<npart;i++) {
//        for(k=0;k<3;k++) {
           v[i][0] += (f[i][0]+fm[i][0])*dt*418.4/(2*m[i]);
           v[i][1] += (f[i][1]+fm[i][1])*dt*418.4/(2*m[i]);
           v[i][2] += (f[i][2]+fm[i][2])*dt*418.4/(2*m[i]);
 //       }
    }
    istep++;    //current step
